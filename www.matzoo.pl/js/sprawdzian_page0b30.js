...
function endTest3() {
    ...
    $.ajax({
        type: "POST",
        url: "web/ajax_testpage.php",
        data: "ocen=" + 1 + "&zad_id=" + 11 + ""
    }).done(function (msg) {
        $('#question11').append(msg);
    });
    $.ajax({
        type: "POST",
        url: "web/ajax_testpage.php",
        data: "ocen=" + 1 + "&zad_id=" + 12 + ""
    }).done(function (msg) {
        $('#question12').append(msg);
    });
    $.ajax({
        type: "POST",
        url: "web/ajax_testpage.php",
        data: "ocen=" + 1 + "&zad_id=" + 13 + ""
    }).done(function (msg) {
        $('#question13').append(msg);
    });
    $.ajax({
        type: "POST",
        url: "web/ajax_testpage.php",
        data: "ocen=" + 1 + "&zad_id=" + 14 + ""
    }).done(function (msg) {
        $('#question14').append(msg);
    });
    $.ajax({
        type: "POST",
        url: "web/ajax_testpage.php",
        data: "ocen=" + 1 + "&zad_id=" + 15 + ""
    }).done(function (msg) {
        $('#question15').append(msg);
    });


    
    $('.spr a').attr('onclick','');
    $('#task.zadanie').css('background-image', 'url(/img/bez_palmy.png)');
...
function showTask3(i) {
    ...
    if (next <= tasks_num) {
        ...
        $('.spr_navidown').append('<input type="button" class="button btn btn-lsuccess" onclick="endTest3();" value="OCEŃ TEST" style="float:right" />');
    }
    if (prev != 0) {
