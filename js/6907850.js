...
function los() {
    // los
    let jednosci = Math.round(Math.random() * 8) + 1;
    let dziesiatki = Math.round(Math.random() * 8) + 1;
    let losNumber = dziesiatki * 10 + jednosci;
    ...
    for (let i=0; i<4; i++) {
        ...
        taskDiv.append('<div class="btn-work btn btn-'+colorsTable[thisColor]+' s30 gradient" onclick="oblicz(\''+wynik+'\',\''+tab[i]+'\')" >'+tab[i]+' '+Game.unitTo+'</div> ');
    }
